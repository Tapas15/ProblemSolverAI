...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get rewards by type
  app.get("/api/rewards/type/:type", async (req, res, next) => {
    try {
      ...
      const rewards = await storage.getRewardsByType(type);
      res.json(rewards);
    } catch (error) {
      next(error);
    }
  ...
  app.get("/api/rewards/:id", async (req, res, next) => {
    try {
  ...
  app.post("/api/rewards", async (req, res, next) => {
    try {
      if (!req.isAuthenticated() || !req.user) {
        return res.status(401).send("Unauthorized");
      }
      ...
      if (!req.user.role || req.user.role !== "admin") {
        return res.status(403).send("Forbidden - Only admins can create rewards");
      }
      
      const validatedData = insertRewardSchema.parse(req.body);
      const reward = await storage.createReward(validatedData);
      
      // Notify connected clients about the new reward
      // (Future enhancement)
  ...
  app.put("/api/rewards/:id", async (req, res, next) => {
    try {
      if (!req.isAuthenticated() || !req.user) {
        return res.status(401).send("Unauthorized");
      }
      ...
      if (!req.user.role || req.user.role !== "admin") {
        return res.status(403).send("Forbidden - Only admins can update rewards");
      }
  ...
  app.delete("/api/rewards/:id", async (req, res, next) => {
    try {
      if (!req.isAuthenticated() || !req.user) {
        return res.status(401).send("Unauthorized");
      }
      ...
      if (!req.user.role || req.user.role !== "admin") {
        return res.status(403).send("Forbidden - Only admins can delete rewards");
      }
  ...
  // Get user rewards
  app.get("/api/user/rewards", async (req, res, next) => {
    try {
      if (!req.isAuthenticated() || !req.user) {
        return res.status(401).send("Unauthorized");
      }
      ...
      // Enhance the response with the full reward details
      const enhancedRewards = await Promise.all(
  ...
  app.get("/api/user/rewards/type/:type", async (req, res, next) => {
    try {
      if (!req.isAuthenticated() || !req.user) {
        return res.status(401).send("Unauthorized");
      }
      ...
      const userRewards = await storage.getUserRewardsByType(req.user.id, type);
  ...
  // Award a reward to a user (can be triggered by system or admin)
  app.post("/api/user/rewards", async (req, res, next) => {
    try {
      if (!req.isAuthenticated() || !req.user) {
        return res.status(401).send("Unauthorized");
      }
      
      // Only admins can award rewards directly, otherwise it's handled by internal logic
      const isAdmin = req.user.role === "admin";
      const targetUserId = isAdmin && req.body.userId ? parseInt(req.body.userId) : req.user.id;
      ...
      const alreadyHasReward = await storage.checkUserRewardExists(targetUserId, rewardId);
      if (alreadyHasReward && !req.body.allowDuplicates) {
        return res.status(409).send("User already has this reward");
      }
      ...
      // If this is a streak-related activity, update the user's streak
      if (req.body.updateStreak) {
        await storage.checkAndUpdateStreak(targetUserId);
      }
      
      // Return the full reward details along with the user reward
      res.status(201).json({
        ...userReward,
        reward
      });
  ...
  // Get user streak
  app.get("/api/user/streak", async (req, res, next) => {
    try {
      if (!req.isAuthenticated() || !req.user) {
        return res.status(401).send("Unauthorized");
      }
      ...
      if (!streak) {
        // No streak exists yet, return a default
        return res.json({
          userId: req.user.id,
          currentStreak: 0,
          longestStreak: 0,
  ...
  app.post("/api/user/streak/check-in", async (req, res, next) => {
    try {
      if (!req.isAuthenticated() || !req.user) {
        return res.status(401).send("Unauthorized");
      }
      ...
      // Update the streak
      const updatedStreak = await storage.checkAndUpdateStreak(req.user.id);
      
      // Check if special streak rewards should be awarded
      // For example, 7-day streak, 30-day streak, etc.
      if (updatedStreak.currentStreak === 7 || 
          updatedStreak.currentStreak === 30 || 
          updatedStreak.currentStreak === 100) {
        
        // Find the appropriate streak reward
        const streakRewards = await storage.getRewardsByType('streak');
        let streakReward;
        
        for (const reward of streakRewards) {
          try {
            const condition = JSON.parse(reward.condition);
            if (condition.days === updatedStreak.currentStreak) {
              streakReward = reward;
              break;
            }
          } catch (e) {
            console.error('Invalid reward condition format', e);
          }
        }
        
        // Award the streak reward if found
        if (streakReward) {
          const alreadyHasReward = await storage.checkUserRewardExists(req.user.id, streakReward.id);
          if (!alreadyHasReward) {
            await storage.createUserReward({
              userId: req.user.id,
              rewardId: streakReward.id,
              data: JSON.stringify({ streakDays: updatedStreak.currentStreak })
            });
          }
        }
      }
