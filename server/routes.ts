...
import { generateQRCode, generateQRCodeFile } from "./services/generate-qr";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Direct routes for certificate images
  app.get('/api/images/fp-logo-new.png', (req, res) => {
    res.sendFile(path.join(process.cwd(), 'public/images/fp-logo-new.png'));
  });

  app.get('/api/images/certified-badge.png', (req, res) => {
    res.sendFile(path.join(process.cwd(), 'public/images/certified-badge.png'));
  });

  app.get('/api/images/manas-signature.png', (req, res) => {
    res.sendFile(path.join(process.cwd(), 'public/images/manas-signature.png'));
  });

  app.get('/api/images/fp-logo-certificates.jpg', (req, res) => {
    // Set caching headers for better performance
    res.setHeader('Cache-Control', 'public, max-age=86400'); // Cache for 24 hours
    res.setHeader('Expires', new Date(Date.now() + 86400000).toUTCString());
    res.sendFile(path.join(process.cwd(), 'public/images/fp-logo-certificates-optimized.jpg'));
  });
  ...
  app.get("/api/certificates/verify/:id", async (req, res, next) => {
    try {
      ...
      // Check if the certificate is revoked (status === 'revoked')
      if (certificate.status === 'revoked') {
        return res.status(200).json({ 
          ...
          certificate: {
            id: certificate.id,
            frameworkName: certificate.frameworkName || "Unknown Framework",
            userName: certificate.userName || "Unknown User",
            issueDate: certificate.issueDate,
            status: certificate.status,
            revokedReason: certificate.revokedReason || "No reason provided"
          }
        });
      }
      ...
      res.status(200).json({
        ...
        certificate: {
          id: certificate.id,
          frameworkName: certificate.frameworkName || (framework ? framework.name : "Unknown Framework"),
  ...
  app.get("/api/certificates/:id/qr", async (req, res, next) => {
    try {
      ...
      if (certificate.userId !== req.user.id) {
        return res.status(403).json({ message: "You do not have permission to access this certificate" });
      }

      // Base URL for verification
      const baseUrl = process.env.BASE_URL || `${req.protocol}://${req.get('host')}`;
      const verificationUrl = `${baseUrl}/api/certificates/verify/${certificate.id}`;

      // Get framework for styling
      const framework = await storage.getFramework(certificate.frameworkId);

      // Get framework-specific styling for QR code
      ...
      const accentColor = framework ? getAccentColor(framework.name) : '#D4AF37';

      // Generate QR code with framework-specific color
...
app.delete("/api/certificates/:id/revoke", async (req, res, next) => {
  ...
  app.get("/api/certificates/:id/download", async (req, res, next) => {
    try {
      ...
      const logoPath = 'public/images/fp-logo-new.png';
      const signaturePath = 'public/images/manas-signature.png';
      const badgePath = 'public/images/certified-badge.png';
      ...
      try {
        ...
        badgeBase64 = `data:image/png;base64,${badgeData.toString('base64')}`;
      } catch (err) {
        console.error('Error reading image files:', err);
        // Fall back to using URLs if files can't be read
        logoBase64 = '/api/images/fp-logo-new.png';
        signatureBase64 = '/api/images/manas-signature.png';
        badgeBase64 = '/api/images/certified-badge.png';
      }
      ...
      <head>
        ...
        <style>
          ...
          .certificate {
            width: 1123px; /* A4 landscape width in px (297mm) */
            height: 794px; /* A4 landscape height in px (210mm) */
          ...
          <div class="inner-content">
            ...
            <div class="content">
              <div style="display: flex; align-items: center; justify-content: center; gap: 15px; margin-bottom: 20px;">
                ...
                <img src="${badgeBase64}" alt="Certified Professional" width="40" height="40" 
