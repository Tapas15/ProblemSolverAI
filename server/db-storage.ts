...
export class PostgresStorage implements IStorage {
  ...
  async getUser(id: number): Promise<User | undefined> {
    const results = await this.db.select().from(users).where(eq(users.id, id)).execute();
    return results[0];
  }

  async getUserByUsername(username: string): Promise<User | undefined> {
    const results = await this.db.select().from(users).where(eq(users.username, username)).execute();
    return results[0];
  }
  
  async getUserByEmail(email: string): Promise<User | undefined> {
    const results = await this.db.select().from(users).where(eq(users.email, email)).execute();
    return results[0];
  }

  async createUser(user: InsertUser): Promise<User> {
    const results = await this.db.insert(users).values(user).returning().execute();
    return results[0];
  }
  ...
  async getFramework(id: number): Promise<Framework | undefined> {
    const results = await this.db.select().from(frameworks).where(eq(frameworks.id, id)).execute();
    return results[0];
  }
  
  async getAllFrameworks(): Promise<Framework[]> {
    return await this.db.select().from(frameworks).execute();
  }
  
  async getFrameworkByName(name: string): Promise<Framework | undefined> {
    const results = await this.db.select().from(frameworks).where(eq(frameworks.name, name)).execute();
    return results[0];
  }
  
  async createFramework(framework: InsertFramework): Promise<Framework> {
    const results = await this.db.insert(frameworks).values(framework).returning().execute();
    return results[0];
  }
  ...
  async getModule(id: number): Promise<Module | undefined> {
    const results = await this.db.select().from(modules).where(eq(modules.id, id)).execute();
    return results[0];
  }
  ...
  async createModule(module: InsertModule): Promise<Module> {
    const results = await this.db.insert(modules).values(module).returning().execute();
    return results[0];
  }
  ...
  private async seedFrameworks() {
    try {
      // MECE Framework
      const meceId = await this.createFramework({
        name: "MECE Framework",
      ...
      // Design Thinking
      const designThinkingId = await this.createFramework({
      ...
      // SWOT Analysis
      const swotId = await this.createFramework({
        name: "SWOT Analysis",
      ...
      const firstPrinciplesId = await this.createFramework({
      ...
      const portersFiveForcesId = await this.createFramework({
      ...
      // Jobs-To-Be-Done
      const jobsToBeDoneId = await this.createFramework({
      ...
      // Blue Ocean Strategy
      const blueOceanId = await this.createFramework({
      ...
      // SCAMPER
      const scamperId = await this.createFramework({
        name: "SCAMPER",
      ...
      const problemTreeId = await this.createFramework({
      ...
      // Pareto Principle
      const paretoId = await this.createFramework({
